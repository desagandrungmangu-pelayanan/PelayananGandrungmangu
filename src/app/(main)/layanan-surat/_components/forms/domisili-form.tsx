...
export function DomisiliForm({ isAdmin = false }: { isAdmin?: boolean }) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-10">
        ...
        <FormSection title="Data Alamat" icon={<User className="h-5 w-5 text-primary" />}>
          ...
          <FormField control={form.control} name="domicileAddress" render={({ field }) => (
            <FormItem className="md:col-span-2">
              ...
              <FormControl><Textarea placeholder="Alamat tempat tinggal saat ini di Desa Gandrungmangu" {...field} disabled={isSubmitting} className="uppercase rounded-2xl" /></FormControl>
        ...
        <Button type="submit" size="lg" disabled={isSubmitting} className="w-full h-16 rounded-2xl bg-primary text-white font-black uppercase tracking-[0.2em] shadow-xl shadow-primary/20 hover:scale-[1.01] transition-all active:scale-95">
          {isSubmitting ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : 'Ajukan Surat Keterangan Domisili'}
        </Button>
      </form>
    </Form>
  );
