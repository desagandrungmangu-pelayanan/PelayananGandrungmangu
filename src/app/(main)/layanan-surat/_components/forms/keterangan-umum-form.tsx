...
import { z } from 'zod';
import { Loader2, User, UploadCloud, FileCheck, FileText } from 'lucide-react';
...
const formSchema = z.object({
  ...
  purpose: z.string().min(1, 'Keperluan surat wajib diisi.'),
...
export function KeteranganUmumForm({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      await addSubmission(firestore, {
        ...
        nik: values.nik,
        letterType: 'Surat Keterangan Umum',
        formData: values,
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-10">
        <FormSection title="Data Identitas & Keperluan" icon={User}>
          ...
          <FormField control={form.control} name="purpose" render={({ field }) => (
            <FormItem className="md:col-span-2">
              ...
              <FormControl>
                <Input placeholder="Contoh: Pengurusan Jamsostek / Persyaratan Administrasi" {...field} disabled={isSubmitting} className="h-12 rounded-xl" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )} />
          ...
          <FormField control={form.control} name="job" render={({ field }) => (
            <FormItem><FormLabel>Pekerjaan</FormLabel><FormControl><Input placeholder="Pekerjaan saat ini" {...field} disabled={isSubmitting} className="h-12 rounded-xl" /></FormControl><FormMessage /></FormItem>
          )} />

          <FormField control={form.control} name="address" render={({ field }) => (
            <FormItem className="md:col-span-2"><FormLabel>Alamat Lengkap Sesuai KTP</FormLabel><FormControl><Textarea placeholder="Alamat lengkap sesuai KTP" {...field} disabled={isSubmitting} className="uppercase rounded-2xl" /></FormControl><FormMessage /></FormItem>
          )} />
        </FormSection>
        ...
        <Button type="submit" size="lg" disabled={isSubmitting} className="w-full h-16 rounded-2xl bg-primary text-white font-black uppercase tracking-[0.2em] shadow-xl shadow-primary/20 hover:scale-[1.01] transition-all active:scale-95">
          {isSubmitting ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : 'Ajukan Surat Keterangan Umum'}
        </Button>
      </form>
    </Form>
  );
